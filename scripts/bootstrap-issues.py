...
EPICS = [
    epic(
        ...
        prs=["9"],
        customer_visible="no",
        intro="Set up the AI layer skeleton in the .NET backend. Ship the full client wrapper (logging, retry, validation, content filtering, cost calc), usage logging schema, spend-cap enforcement, persisted content-filter outcomes, first 3 context blocks (brand/org/industry), email-generate proof of pattern, multi-tenant isolation, and prompt management.",
            ...
            story("1.1", "Create AI Project Structure",
                  ...
                  labels=slabels(1, ["9"]), milestone=M1, tasks=[
                      task("1.1.1", "Create AI Domain Project", [
                          ...
                          "Set up folder structure: Client/, Prompts/, Context/, Knowledge/, WebAgent/, Workflows/, Schemas/, Usage/",
            ...
            story("1.2", "Client Wrapper Service (IAiClient)",
                  ...
                  labels=slabels(1, ["9"]), milestone=M1, tasks=[
                      ...
                      task("1.2.5", "Azure AI Foundry Provider Adapter", [
                          "Implement Azure AI Foundry adapter behind IAiClient without changing feature callers",
                          "Map model classes (generation, classification, reasoning) to Foundry deployment names",
                          "Update provider-specific pricing/cost tables for Foundry deployments",
                          "Verify usage logging, spend caps, content filtering, validation retries, and graceful fallback still work through the adapter",
                          "Add side-by-side quality tests for Anthropic direct vs Foundry output on representative prompts",
                          "Keep this task provider-readiness/post-launch unless Foundry is explicitly promoted to launch dependency",
                      ], labels=tlabels(1, "9"), milestone=M1, depends="Task 1.2.1, Task 1.3.2, Task 1.3.3"),
                  ]),
            story("1.3", "Usage Logging & Cost Tracking",
                  ...
                  labels=slabels(1, ["9"]), milestone=M1, tasks=[
                      ...
                      task("1.3.3", "Spend Cap Enforcement", [
                          "Add a pre-provider spend-cap check in the AI client pipeline",
                          "Support per-account daily cap and a default configured cap",
                          "Calculate current-day spend from ssp.ai_usage before provider invocation",
                          "Return a typed AI client error when capped",
                          "Add tests proving capped calls do not invoke the provider",
                      ], labels=tlabels(1, "9"), milestone=M1, depends="Task 1.3.2, Task 1.2.1"),
                      task("1.3.4", "Persist Content Filter Outcome", [
                          "Extend the content-filter contract to return structured outcome metadata",
                          "Persist PII categories, banned-term categories/severities, action taken, and direction to ai_usage.content_filter_outcome",
                          "Ensure sensitive matched substrings are never logged or persisted",
                          "Add tests for redacted output and rejected input/output paths",
                      ], labels=tlabels(1, "9"), milestone=M1, depends="Task 1.3.2, Task 1.2.4"),
                  ]),
            story("1.4", "First 3 Context Blocks (brand, org, industry)",
                  ...
                  labels=slabels(1, ["9"]), milestone=M1, tasks=[
                      task("1.4.1", "Context Block Base", [
                          "IContextBlock<T> interface: LoadBlockContextAsync(accountId, options?) → structured context object",
                          "ToPromptString() method on context objects",
                          "Graceful degradation: missing/sparse data → empty stub, never throw",
                          "Define a shared Exists / empty-context convention for all AI context objects",
                          "Document the prototype-loader-to-Sweetspot-context mapping from docs/ai-arch-scope.md",
                      ], labels=tlabels(1, "9"), milestone=M1, depends="Task 1.1.1"),
                      task("1.4.2", "Brand Context Block", [
                          "Load Brand Kit through the internal Phase 1 read path (EF Core-backed service under Option A)",
                          "Return typed prompt-facing fields: voice_description, tone_tags, values, one_liner, exists",
                          "Return typed renderer-facing fields: colors, fonts, logos, resolved roles, email_footer_html, template_banners",
                          "Ensure ToPromptString() includes tonal fields only and excludes colors, fonts, logos, and raw visual data",
                          "Gracefully return exists = false / empty tonal context when Brand Kit is missing",
                      ], labels=tlabels(1, "9"), milestone=M1, depends="Task 1.4.1"),
                      task("1.4.3", "Org Context Block", [
                          "Load Sweetspot account/org identity through existing tenant conventions",
                          "Include industry key/template, organisation name, compliance/footer basics, and privacy/contact fields required by generated content",
                          "Gracefully fall back when optional org/profile fields are missing",
                      ], labels=tlabels(1, "9"), milestone=M1, depends="Task 1.4.1"),
                      task("1.4.4", "Industry Context Block", [
                          "Load industry info from industry_template_configs table",
                          "Include industry vocabulary, recurring-value labels, examples only when server-side consumers need them",
                          "Return deterministic defaults when no industry config exists",
                      ], labels=tlabels(1, "9"), milestone=M1, depends="Task 1.4.1"),
                  ]),
    ...
    epic(
        eid="3",
        title="Web Agent Cleanup & Field-Tagging UI",
        sprint_range="Sprint 3 (Jun 30 – Jul 11 2026)",
        prs=["11", "11p"],
        customer_visible="partial",
        intro="Form/event confirmation senders stop being treated as agents — journeys take over. Web Agent remains the only customer-facing product agent type. HTML duplication removed. In parallel, ship field-tagging UI: settings page with 3-tier tag rendering, AI suggestion for custom fields, readiness percentage.",
            ...
            story("3.1", "Form/Event Confirmation Handler Migration",
                  ...
                  labels=slabels(3, ["11"]), milestone=M3, tasks=[
                      task("3.1.1", "Product Agent Definition (Web Agent Only)", [
                          'Apply "Agent" = Web Agent: customer-facing multi-turn website/chat experience',
                          "Reclassify single-call email senders, form generators, event generators, strategy workflows, and insights workflows as handlers/generators/workflows",
                          "Update user-facing labels and internal planning language so Form Agent and Event Agent are not treated as product agent types",
                      ], labels=tlabels(3, "11"), milestone=M3, depends="PR 10"),
                      task("3.1.2", "Confirmation Email → Journey Migration", [
                          "Move form-submission and event-registration confirmation email creation logic to journeys",
                      ...
                      task("3.1.3", "Web Agent / Inbox Runtime Narrowing", [
                          "Remove separate Form Agent and Event Agent runtime surfaces",
                          "Route any needed inbound conversation handling through the Web Agent / inbox conversation model with original registration context",
                          "Remove HTML generation duplication (email-generate / form confirmation / event confirmation)",
                          'Simplify legacy form/event triggers to "enrol in journey + open conversation thread when needed"',
    ...
    epic(
        eid="4",
        title="Context Blocks + AI Surface Parity Wiring",
        sprint_range="Sprint 4 (Jul 14 – Jul 25 2026)",
        prs=["12"],
        customer_visible="no",
        intro="Build the remaining 7 context block services: methodology, plan, field-semantics, audience, insights, knowledge (generalised from Web Agent-only to all eligible features), assets. Wire up feature → block composition registry and explicit surface contracts for Forms v2, field mapping, landing pages, Events Create-with-AI, Web Agent, profile research, insights/social/inbound classification, and feature-key/invariant registry. Decision checkpoint at Week 8: has field-tagging UI shipped and are users tagging?",
            ...
            story("4.3", "Insights & Knowledge Context Blocks",
                  ...
                  labels=slabels(4, ["12"]), milestone=M4, tasks=[
                      ...
                      task("4.3.2", "Knowledge Context Block", [
                          ...
                          "Generalize knowledge block from Web Agent-only to all eligible features",
                      ], labels=tlabels(4, "12"), milestone=M4, depends="PR 9"),
                  ]),
            story("4.4", "Assets Context Block",
                  ...
                  labels=slabels(4, ["12"]), milestone=M4, tasks=[
                      task("4.4.1", "Assets Context Block", [
                          "Load enriched asset metadata from content_assets table",
                          "Include only selected analysed metadata: ai_description, dominant_colors, suggested_use, tone, is_analysed",
                          "Exclude raw files, full libraries, and unanalysed assets from prompts",
                          "Document that semantic asset search waits for embeddings / pgvector",
                      ], labels=tlabels(4, "12"), milestone=M4, depends="PR 9"),
                  ]),
            story("4.5", "Feature → Block Composition Wiring",
                  ...
                  labels=slabels(4, ["12"]), milestone=M4, tasks=[
                      task("4.5.1", "Composition Registry", [
                          "Feature → blocks mapping registry (email-generate, journey-content, form-generate, Web Agent, strategy workflow, insights workflow, recommendation)",
            ...
            story("4.6", "Prototype AI Surface Parity Wiring",
                  ...
                  labels=slabels(4, ["12"]), milestone=M4, tasks=[
                      task("4.6.1", "Forms v2 Generation + Refinement Wiring", [
                          "Port the Forms v2 generate/refine prompt contracts into .NET prompt classes",
                          "Use brand + org + industry + profile + contact field context",
                          "Validate against a typed GeneratedFormDefinition contract",
                          "Define feature keys for form generation and refinement; do not leave Forms v2 on a legacy usage shim",
                          "Confirm this is a structural JSON generator, not an agent",
                      ], labels=tlabels(4, "12"), milestone=M4, depends="Task 4.5.1, Task 4.2.1"),
                      task("4.6.2", "Forms v2 Field Mapping Wiring", [
                          "Preserve exact-match and synonym mapping before the AI call",
                          "Use AI only for unresolved captured-form fields",
                          "Return map_existing, create_new, or dont_capture with confidence per field",
                          "Batch unmapped fields into one classification call",
                          "Define the forms_v2_field_mapping feature key and model class",
                      ], labels=tlabels(4, "12"), milestone=M4, depends="Task 4.2.1"),
                      task("4.6.3", "Landing Page Generation, Refinement, and Layout Switching Wiring", [
                          "Port landing-page generate/refine/fill-missing-block prompt contracts",
                          "Use relaxed generation schemas and server-stamped IDs for page blocks",
                          "Preserve the rule that AI does not emit URLs, page block IDs, or image asset IDs",
                          "Preserve layout-switch missing-blocks-only behavior and bag-of-blocks storage model",
                          "Define feature keys for generation, refinement, and layout switch",
                      ], labels=tlabels(4, "12"), milestone=M4, depends="Task 4.5.1, Task 4.4.1"),
                      task("4.6.4", "Events Create-with-AI Surface Contract", [
                          "Define event field extraction contract with nullable operational basics and marketing brief fields",
                          "Define event journey structure contract for Invitations, Confirm, and Followup journeys",
                          "Preserve cta_intent indirection so AI never emits raw URLs",
                          "Preserve slot-based scaffold rendering for event invitations and HTML rendering for followups",
                          "Define feature keys for event extraction, journey structure, invite content, followup content, and regeneration",
                      ], labels=tlabels(4, "12"), milestone=M4, depends="PR 10, Task 4.5.1"),
                      task("4.6.5", "Web Agent Context and Knowledge Wiring", [
                          "Rename customer-facing chat agent references to Web Agent",
                          "Compose brand + org + industry + knowledge + conversation history",
                          "Preserve response-length configuration and fallback-message behavior",
                          "Confirm Web Agent is the only customer-facing product agent type",
                      ], labels=tlabels(4, "12"), milestone=M4, depends="Task 4.3.2, Task 4.5.1"),
                      task("4.6.6", "Profile Research, Insights Ask, Social Generate, and Inbound Classification Contracts", [
                          "Document profile research as an onboarding extraction workflow with nullable per-field outputs",
                          "Document insights ask as org-data Q&A with tenant-scoped context and traceable usage",
                          "Document social generation as a content-generation workflow using the shared voice floor",
                          "Document inbound classification as a classification workflow with explicit feature key and model class",
                          "Mark each workflow as launch, post-launch, or explicit product deferral before PR 13 starts",
                      ], labels=tlabels(4, "12"), milestone=M4, depends="Task 4.5.1"),
                      task("4.6.7", "AI Feature Key and Invariant Registry", [
                          "Create a single registry of feature keys, model classes, prompt class, schema contract, context composition, and launch/deferred status for every AI surface",
                          "Include the handover invariants: every call logs usage, spend cap before provider call, voice floor on content prompts, dash stripping on text output, schema validation, server-stamped IDs, no AI-emitted URLs, graceful failure, org/account scope on every call",
                          "Add Forms v2 feature keys that are TBD in the handover",
                          "Use the registry as the PR 13 readiness gate",
                      ], labels=tlabels(4, "12"), milestone=M4, depends="Tasks 4.6.1, 4.6.2, 4.6.3, 4.6.4, 4.6.5, 4.6.6"),
                  ]),
        ],
    ),
